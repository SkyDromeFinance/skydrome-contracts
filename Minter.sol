...
import "contracts/interfaces/ISky.sol";
...
contract Minter is IMinter {
    ...
    ISky public immutable _sky;
    IVoter public immutable _voter;
    IVotingEscrow public immutable _ve;
    IRewardsDistributor public immutable _rewards_distributor;
    uint public weekly = 13_000_000 * 1e18; // represents a starting weekly emission of 13M SKY (SKY has 18 decimals)
    ...
    function circulating_supply() public view returns (uint) {
        return _sky.totalSupply() - _ve.totalSupply();
    }
    ...
    function calculate_growth(uint _minted) public view returns (uint) {
        ...
        uint _flowTotal = _sky.totalSupply();
        return
    ...
    function update_period() external returns (uint) {
        ...
        if (block.timestamp >= _period + WEEK && initializer == address(0)) {
            ...
            require(_sky.transfer(address(_rewards_distributor), _growth));
            _rewards_distributor.checkpoint_token(); // checkpoint token balance that was just minted in rewards distributor
            _rewards_distributor.checkpoint_total_supply(); // checkpoint supply

            _sky.approve(address(_voter), weekly);
