...
contract Vestor is Ownable {
    ...
    /**
     * @dev Initializes the SKY token address
     *
    ...
    function claim() external returns (uint256 vestedAmount) {
        ...
        SKY.transfer(msg.sender, vestedAmount);
    }
    ...
    function cancelVest(address user) external onlyOwner {
        ...
        require(
            SKY.balanceOf(address(this)) >= amount,
            "Insufficient SKY balance"
        );
        delete vest[user];
        SKY.transfer(revokeTo, amount);
